...
import io.bootique.swagger.redoc.model.RedocServletTemplateModel;
 ...
 * @since 2.0
 */
public class RedocServlet extends StaticServlet {

    static final String PATH_INFO_ONLY_PARAMETER = "pathInfoOnly";

    private Mustache template;
    private Map<String, RedocServletModel> models;

    public RedocServlet(String resourceBase, Mustache template, Map<String, RedocServletModel> models) {
        super(resourceBase);
    ...
    protected void doGetConsole(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        RedocServletModel model = models.get(servletPath);
        if (model == null) {
        ...
        RedocServletTemplateModel templateModel = model.createTemplateModel(request);
